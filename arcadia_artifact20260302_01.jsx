...
const ASSET_STATUS = {
  "title/title_bg":          true,
  "scenes/s00_vrs":          false,
  "scenes/s01_beach":        false,
  "scenes/s02_coast":        false,
  "scenes/s03_village":      false,
  "scenes/s04_guild":        false,
  "scenes/s07_meadow":       false,
  "scenes/s09_inn":          false,
  "scenes/s10_tavern":       false,
  "scenes/s11_coast2":       false,
  "scenes/s13_market":       false,
  "scenes/s14_rocks":        false,
  "scenes/s17_armory":       false,
  "scenes/s19_pier":         false,
  "scenes/s20_guild2":       false,
  "scenes/s25_westcoast":    false,
  "scenes/s26_cave_blue":    false,
  "scenes/s27_cave_deep":    false,
  ...
  "sprites/swift":           false,
  "sprites/linz":            false,
  "sprites/chopper":         false,
  "sprites/cricket":         false,
  "sprites/koza_sp":         false,
  "sprites/rose":            false,
  "sprites/juda":            false,
  "sprites/ymir":            false,
  "sprites/rubens":          false,
...
export default function Arcadia() {
  ...
  const [battleEnemyKey, setBattleEnemyKey] = useState(null);
  ...
  const sceneImgUrl = sceneImgKey ? assetUrl(sceneImgKey) : null;
  const bgStyle = sceneImgUrl
    ? { backgroundImage: `url(${sceneImgUrl})`, backgroundSize: "cover", backgroundPosition: "center top" }
  ...
  if (phase === "title") {
    const titleBgUrl = assetUrl("title/title_bg");
    const titleBgStyle = titleBgUrl
      ? { backgroundImage: `url(${titleBgUrl})`, backgroundSize: "cover", backgroundPosition: "center" }
      : { background: "linear-gradient(180deg,#020810 0%,#050d14 40%,#0a1828 100%)" };
    return (
    <div style={{width:"100%",height:"100%",minHeight:"600px",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",...titleBgStyle,fontFamily:"'Noto Serif JP',serif",position:"relative",overflow:"hidden"}}>
  ...
  if (phase === "battle") {
    ...
    const btlBgKey = BATTLE_BG_MAP[battleEnemyKey];
    const btlBgUrl = btlBgKey ? assetUrl(btlBgKey) : null;
    const btlBgStyle = btlBgUrl
      ? { backgroundImage: `url(${btlBgUrl})`, backgroundSize: "cover", backgroundPosition: "center" }
      : { background: `linear-gradient(180deg,${ed.bg[0]} 0%,${ed.bg[1]} 50%,${ed.bg[2]} 100%)` };

    const enemyImgKey = ENEMY_IMG_MAP[battleEnemyKey];
    const enemyImgUrl = enemyImgKey ? assetUrl(enemyImgKey) : null;

    return (
      <div style={{width:"100%",height:"100%",minHeight:"600px",display:"flex",flexDirection:"column",...btlBgStyle,fontFamily:"'Noto Serif JP',serif",userSelect:"none",position:"relative",overflow:"hidden"}}>
        <style>{keyframes}</style>
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"20px 20px 10px"}}>
          ...
          {enemyImgUrl
            ? <img src={enemyImgUrl} alt={ed.name} style={{width:isBoss?220:140,height:isBoss?220:140,objectFit:"contain",animation:isBoss?"bossFloat 2s infinite":"idle 2s infinite",filter:isBoss?`drop-shadow(0 0 20px ${C.red})`:"drop-shadow(0 0 8px rgba(0,200,255,0.3))",transform:btlAnimEnemy?"scale(1.1)":"scale(1)",transition:"transform 0.1s"}}/>
          ...
          {!victory && !defeat ? (
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:8}}>
              {BATTLE_SKILLS.map(sk => {
                const skillImgUrl = assetUrl(SKILL_IMG_MAP[sk.id]);
                return (
                  <button key={sk.id} onClick={() => doBattleAction(sk.id)}
                    ...
                    {skillImgUrl
                      ? <img src={skillImgUrl} alt={sk.label} style={{width:20,height:20,objectFit:"contain"}}/>
    ...
    <div style={{width:"100%",height:"100%",minHeight:"600px",display:"flex",flexDirection:"column",...bgStyle,fontFamily:"'Noto Serif JP',serif",userSelect:"none",position:"relative",overflow:"hidden",transition:"background 1s"}}>
      ...
      <div style={{flex:1,display:"flex",alignItems:"flex-end",justifyContent:"center",padding:"20px 20px 0",position:"relative",zIndex:5,minHeight:200}}>
        ...
        <div style={{display:"flex",gap:16,alignItems:"flex-end",justifyContent:"center",flexWrap:"wrap"}}>
          {sc.sprites.map((sp, i) => {
            const spImgKey = SPRITE_MAP[sp];
            const spImgUrl = spImgKey ? assetUrl(spImgKey) : null;
            const isHero = i === 0;
            return spImgUrl
              ? <img key={i} src={spImgUrl} alt={sp} style={{height:isHero?130:100,objectFit:"contain",animation:`idle ${2+i*0.3}s ${i*0.2}s infinite`,filter:isHero?"drop-shadow(0 0 8px rgba(0,200,255,0.3))":"none"}}/>
              : <div key={i} style={{fontSize:isHero?52:40,animation:`idle ${2+i*0.3}s ${i*0.2}s infinite`,filter:isHero?"drop-shadow(0 0 8px rgba(0,200,255,0.3))":"none",textShadow:"0 4px 8px rgba(0,0,0,0.5)"}}>{sp}</div>;
          })}
        </div>
      </div>
